...
public class ServiceConnectionEx {
	...
	private IMyInterface mService = null;
	...
	public IMyInterface getService() {
	...
	public void bindService() {
		...
		try {
			...
			if (!mContext.bindService(intent, mServiceConnection, Context.BIND_AUTO_CREATE)) {
				mIsConnecting = false;
//	TODO			showMessage("bindServiceFailed");
				Log.e(Constants.TAG, "[App] [java] bindServiceFailed. " + "Name: " + mServiceName);
//				throw new RuntimeException("bindServiceFailed: " + mServiceName);
			}
	...
	private final ServiceConnection mServiceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder binder) {
			...
			mService = IMyInterface.Stub.asInterface(binder);
			mIsConnected = true;
			mIsConnecting = false;
			mWaitLock.open();
		}
